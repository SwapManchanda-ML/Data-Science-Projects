    ...
    **YOU MUST COMPLETE THIS CLASS**
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        def _one_step_lookahead(s,V):
            A = np.zeros(self.action_dim)
            for a in range(self.action_dim):
                '''
                next_state = -1 
                for R,p in zip(self.mdp.R[s,a],self.mdp.P[s,a]):
                    next_state = next_state + 1
                    #if p > 0:
                    A[a] += p * (R + self.gamma * V[next_state])
                '''
                for s1 in range(self.state_dim):
                    A[a] += self.mdp.P[s,a,s1]*(self.mdp.R[s,a,s1] + self.gamma * V[s1])
    ...
    def _policy_eval(self, policy: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for s in range(self.state_dim):
                v = 0
                for a, next_action_prob in enumerate(policy[s]):
                    #print(a,next_action_prob)
                    for s1 in range(self.state_dim):
                        v += next_action_prob * self.mdp.P[s,a,s1] * (self.mdp.R[s,a,s1] + self.gamma * V[s1])
                delta = max(delta, np.abs(v - V[s]))
                V[s] = v
            if delta < self.theta:
                break
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for s in range(self.state_dim):
                ...
                best_action =  np.argmax(_one_step_lookahead(s,V))
...
if __name__ == "__main__":
    ...
    mdp.add_transition(
        ...
        Transition("high", "wait", "high", 1, 2), #2
        Transition("high", "search", "high", 0.8, 5), #5
        Transition("high", "search", "low", 0.2, 5), #5
        Transition("high", "recharge", "high", 1, 0), #0
        Transition("low", "recharge", "high", 1, 0), #0
        Transition("low", "wait", "low", 1, 2), #2
        Transition("low", "search", "high", 0.6, -3), #-3
        Transition("low", "search", "low", 0.4, 5), #5
    )
